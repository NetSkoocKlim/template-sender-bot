...
from app.bot.src.keyboards.admin.constants import AdminPanelOptions, AdminPanelStatisticOptions, MailingInfoOptions
from app.bot.src.keyboards.admin.fabrics import DownloadMailingData, PaginateButtonData, MailingViewData, \
    MailingTemplateViewData
from app.bot.src.keyboards.admin.menu import (get_admin_panel_statistic_menu_kb, get_mailings_inline_kb,
...
async def handle_download_mailing(
        callback: CallbackQuery,
        callback_data: DownloadMailingData,
        s3_storage: ObjectStorage
):
    mailing_key = callback_data.key
    data, filename = await s3_storage.download_file(mailing_key)
    filename = filename or f"mailing-{datetime.datetime.now().timestamp()}.csv"

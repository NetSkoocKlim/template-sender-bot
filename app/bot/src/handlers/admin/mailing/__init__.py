...
from app.bot.src.handlers.admin.template.edit import get_templates_list
from app.bot.src.keyboards.admin.constants import AdminPanelOptions, AdminPanelMailingOptions, AdminPanelChosenTemplateOptions

from app.bot.src.keyboards.admin.menu import (get_admin_panel_mailing_menu_kb,
...
from shared.src.database.models import User, Template, Mailing, Receiver
from redis.asyncio import Redis

from app.bot.src.utils import create_mailing_result_csv
...
async def handle_begin_mailing_button(callback: CallbackQuery, admin: User,
    ...
    created_mailing = await Mailing.create(
        session=session,
        admin_id=admin.id,
    ...
    try:
        ...
        await mailing_sender.upload_mailing_command(
            UploadMailingCommand(
                sender_id=admin.id,
                mailing_id=created_mailing.id,
                file_name="random_file_name",
                file_bytes_b64=base64.b64encode(csv_bytes.getvalue()).decode('utf-8'),
            )
        )

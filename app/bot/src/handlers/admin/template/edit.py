...
from app.bot.src.handlers.admin.template.add import MAX_TEMPLATE_NAME_LENGTH, MAX_TEMPLATE_DESCRIPTION_LENGTH
from app.bot.src.keyboards.admin.constants import AdminPanelTemplateOptions
from app.bot.src.keyboards.admin.fabrics import PaginateButtonData, TemplateEditData, TemplateEditAction
from app.bot.src.keyboards.admin.menu import (get_templates_inline_kb, get_template_edit_inline_kb,
...
from app.bot.src.states.states import TemplateEditStates
from app.bot.src.utils.copy_message import copy_text_message
from shared.src.database.models import Template, User
from shared.src.database import TemplatePaginator
...
async def handle_new_template_name(message: Message, state: FSMContext, session: AsyncSession):
    ...
    if len(new_template_name) > MAX_TEMPLATE_NAME_LENGTH:
        await message.bot.send_message(
            text=f"Длина названия не должна превышать {MAX_TEMPLATE_NAME_LENGTH}. Придумайте другое название",

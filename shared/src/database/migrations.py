...
def _get_alembic_config() -> Config:
    ...
    cfg.set_main_option('sqlalchemy.url', settings.db.URL)
...
async def _needs_auto_stamp() -> bool:
    ...
    from shared.src.database.db_helper import DBHelper

    async with DBHelper.engine.connect() as conn:
...
async def run_alembic_upgrade() -> None:
    ...
    try:
        ...
        logger.exception("Ошибка при выполнении миграций:")
...
async def _stamp_alembic_revision(revision: str) -> None:
    ...
    logger.info('Alembic: база отмечена как актуальная. Ревизия: %s', revision)